...
contract('dBank', ([deployer, user]) => {
  ...
  //pass the mint role to the bank test
  beforeEach(async () => {
  ...
  //test the token address and check the name, symbol, total supply 
  describe('testing token contract...', () => {
    describe('success', () => {
      it('checking token name', async () => {
        expect(await token.name()).to.be.eq('Decentralized Bank Currency') //expect the name to be Decentralized bank currency 
      })

      it('checking token symbol', async () => {
        expect(await token.symbol()).to.be.eq('DBC') //expect the symbol to be this
      })
      ...
      it('dBank should have Token minter role', async () => {
        expect(await token.minter()).to.eq(dbank.address) //expect the minter role to be the dbank
      })
    })

            //chech to make sure minter role fails when other than dbank and revert if it fails
    describe('failure', () => {
      ...
      //check balance will call deposit function 0.01 eth then increase bank balance
  ...
  describe('testing withdraw...', () => {
    let balance
        ...
        //test to make sure we can withdraw
    describe('success', () => {
